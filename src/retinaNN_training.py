...
from keras.layers import Input, Convolution2D, MaxPooling2D, UpSampling2D, Reshape, core, Dropout, Dense, Flatten
...
#Define the neural network with final autoencoder (not working cannot use the toReconstruct output for the decoder)
def get_unet_autoenc(n_ch,patch_height,patch_width):
    inputs = Input(( patch_height, patch_width, n_ch))
    conv1 = Convolution2D(32, 3, 3, activation='relu', border_mode='same')(inputs)
    conv1 = Dropout(0.2)(conv1)
    conv1 = Convolution2D(32, 3, 3, activation='relu', border_mode='same')(conv1)
    ...
    # add autoencoding to smallest layer
    smallFlatConv = Flatten(name='toReconstuct')(conv3)
    encoded = Dense(128, activation='relu')(smallFlatConv)
    encoded = Dense(64, activation='relu')(encoded)
    encoded = Dense(32, activation='relu', name='fullyEncoded')(encoded)

    decoded = Dense(64, activation='relu')(encoded)
    decoded = Dense(128, activation='relu')(decoded)
    decoded = Dense(18432, activation='sigmoid', name='decoderOutput')(decoded) # size of smallFlatConv TODO: check
    ...
    conv6 = core.Permute((2,1))(conv6)
    ############
    conv7 = core.Activation('softmax', name='segOutput')(conv6)

    model = Model(input=inputs, output=[conv7, decoded])
...
def get_unet_autoenc2(n_ch,patch_height,patch_width):
    inputs = Input(( patch_height, patch_width, n_ch))
    conv1 = Convolution2D(32, 3, 3, activation='relu', border_mode='same')(inputs)
    conv1 = Dropout(0.2)(conv1)
    conv1 = Convolution2D(32, 3, 3, activation='relu', border_mode='same')(conv1)
    ...
    # # add autoencoding to smallest layer (test5)
    # smallFlatConv = Flatten(name='toReconstuct')(conv3)
    # encoded = Dense(128, activation='relu')(smallFlatConv)
    # encoded = Dense(64, activation='relu')(encoded)
    # encoded = Dense(32, activation='relu', name='fullyEncoded')(encoded)
    ...
    # decoded = Dense(64, activation='relu')(encoded)
    # decoded = Dense(128, activation='relu')(decoded)
    # decoded = Dense(18432, activation='relu', name='decoderOutput')(decoded)
    #
    # conv3b = Reshape((( 12, 12, 128)))(decoded) # same shape as conv3

    # add autoencoding to smallest layer (test6)
    smallFlatConv = Flatten(name='toReconstuct')(conv3)
    encoded = Dense(2048, activation='relu')(smallFlatConv)
    encoded = Dense(128, activation='relu', name='fullyEncoded')(encoded)
    ...
    up1 = concatenate([UpSampling2D(size=(2, 2))(conv3b), conv2], axis=3)
    conv4 = Convolution2D(64, 3, 3, activation='relu', border_mode='same')(up1)
    conv4 = Dropout(0.2)(conv4)
    conv4 = Convolution2D(64, 3, 3, activation='relu', border_mode='same')(conv4)
    #
...
#Define the neural network (original)
def get_unet_bak(n_ch,patch_height,patch_width):
...
# convert to tf
# patches_masks_train = patches_masks_train.transpose( 0, 2, 3, 1 )
...
# # original U-net with auto encoder
# autoencTgt = model.get_layer('toReconstuct')
#
# model.fit(patches_imgs_train, { 'segOutput': patches_masks_train, 'decoderOutput': autoencTgt }, nb_epoch=N_epochs, batch_size=batch_size, verbose=2, shuffle=True, validation_split=0.1, callbacks=[checkpointer])
